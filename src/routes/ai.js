...
import Garment from "../models/Garment.js"; // ⚠️ en ES Modules, il faut l'extension .js
...
const colorAliases = {
  ...
  beige: "beige", creme: "cream", cream: "cream",
  ivory: "ivory", marine: "navy", navy: "navy",
...
const guessGroup = (category) => {
  ...
  for (const [group, keywords] of Object.entries(categoryGroups)) {
    if (keywords.some((word) => value.includes(word))) return group;
  }
  return "unknown";
};
...
const pickItems = (items, group, seedColor, limit = 3) => {
  ...
  return (compatible.length > 0 ? compatible : candidates).slice(0, limit);
};

const targetGroupsForSeed = (seedGroup) => {
  switch (seedGroup) {
    case "top": return ["bottom","shoes","outer"];
    case "bottom": return ["top","shoes","outer"];
    case "dress": return ["shoes","outer","accessory"];
...
router.post("/recommend", async (req, res) => {
  try {
    ...
    let seed = null;
    if (seedId) {
      seed = items.find((item) => item._id.toString() === seedId) || await Garment.findById(seedId).catch(() => null);
    }
...
router.post("/assistant", async (req, res) => {
  try {
    ...
    if (!items.length) {
      return res.json({ message: "Ajoute quelques vêtements pour que je puisse proposer une tenue.", outfit: {} });
    }
    ...
    const message = parts.length ? `Je te propose ${parts.join(", ")}.` : "Je n'ai pas assez de pièces pour composer une tenue.";
